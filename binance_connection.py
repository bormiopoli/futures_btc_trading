...
from binance_credentials import BINANCE_API_KEY, BINANCE_SECRET, MY_SECRET
# binance_client = Client(BINANCE_API_KEY, BINANCE_SECRET)
binance_client = Spot(key=BINANCE_API_KEY, secret=BINANCE_SECRET)

def get_binance_k_lines(ticker, interval, value, all_data={}, myclient=binance_client, n_records=0, end_time=None):

    if n_records == 0:
        n_records = 1000
    if end_time is not None:
        k_values = myclient.klines(symbol=ticker, interval=interval, endTime=int(end_time*1000), limit=n_records)
    else:
        k_values = myclient.klines(symbol=ticker, interval=interval, limit=n_records)
...
def remove_ticker_not_in_binance(my_list, interval='1m', n_records=0, end_time=None):
    ...
    for i, value in enumerate(my_list):
        ...
        try:
            ...
            #binance_client = Client(BINANCE_API_KEY, BINANCE_SECRET)
            #data[value] = get_binance_k_lines(ticker, interval, value, myclient=binance_client, end_time=end_time)
